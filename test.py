...
class Test(TestCase):
    ...
    #Checking for proper json return type
    ...
    #Checking contents of request after proper 200 response from ping
    def test_returned_content_type_info(self):
        tester = app.test_client(self)
        respon = tester.get('/api/posts')
        self.assertEqual(respon.content_type, "text/html; charset=utf-8")

    #Checks and Confirms proper data has been returned
    def test_returns_data_info(self):
        tester = app.test_client(self)
        respon = tester.get('/api/posts')
        self.assertTrue(bytes(Hatch.GetPosts.make_req('science', 'reads', 'asc').encode()) in respon.data)
